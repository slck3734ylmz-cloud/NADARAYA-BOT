    ...
    try:
        ...
        for i, row in df_3d.iterrows():
            for m in [0.99, 0.98, 0.96]:
                ...
                long_liq[p] = long_liq.get(p, 0.0) + row["Hacim"]
            for m in [1.01, 1.02, 1.04]:
                p = round((row["Yuksek"] * m) / round_step) * round_step
                short_liq[p] = short_liq.get(p, 0.0) + row["Hacim"]
...
# Non-Repainting Nadaraya-Watson Filtresi
...
# Plotly Grafik Çizim Fonksiyonu
...
# ================= KÜRESEL DEĞİŞKENLERİN TANIMLANMASI =================
# HATA GİDERİCİ: Global değişkenler en üstte garantiye alındı (NameError çözüldü)
