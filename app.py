...
def nadaraya_watson_estimator(src, h=8):
    ...
    est = np.zeros(n)
    for i in range(n):
        w = np.exp(-((np.arange(n) - i) ** 2) / (2 * h ** 2))
        est[i] = np.sum(src * w) / np.sum(w)
    return est

def calculate_nw_bands(df, std_m, suffix):
    ...
    df[f"NW_Ust{suffix}"] = df["NW_Merkez"] + (std_m * df["Sapma_Std"])
    df[f"NW_Alt{suffix}"] = df["NW_Merkez"] - (std_m * df["Sapma_Std"])
    return df

def draw_plotly_chart(df, p_col, alt_col, ust_col, title, l_avg=0.0, s_avg=0.0):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df["Zaman"], y=df[p_col], name="Anlık Fiyat", line=dict(color='royalblue', width=2)))
    fig.add_trace(go.Scatter(x=df["Zaman"], y=df[ust_col], name="Üst Band", line=dict(color='crimson', width=1.5, dash='dash')))
    fig.add_trace(go.Scatter(x=df["Zaman"], y=df[alt_col], name="Alt Band", line=dict(color='limegreen', width=1.5, dash='dash')))
    if l_avg > 0: fig.add_trace(go.Scatter(x=df["Zaman"], y=[l_avg]*len(df), name="Long Ort.", line=dict(color='green', width=1.5)))
    if s_avg > 0: fig.add_trace(go.Scatter(x=df["Zaman"], y=[s_avg]*len(df), name="Short Ort.", line=dict(color='red', width=1.5)))
    fig.update_layout(title=title, template="plotly_dark", xaxis_title="Zaman", yaxis_title="Fiyat", margin=dict(l=20, r=20, t=40, b=20), height=400)
    return fig

# ================= GLOBAL VERİLER VE NAVİGASYON =================
