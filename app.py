    ...
    try:
        ...
        for symbol, ticker in tickers.items():
            if symbol.endswith(':USDT'):
                quote_vol = ticker.get('quoteVolume') or (ticker.get('baseVolume', 0.0) * (ticker.get('last') or ticker.get('close') or 0.0))
                if quote_vol > 0:
                    usd_tickers.append({
    ...
    try:
        ...
        for symbol, ticker in tickers.items():
            if symbol.endswith(':USDT'):
                price = ticker.get('last') or ticker.get('close') or 0.0
                change = ticker.get('percentage') or 0.0
                fr_val = float(ticker.get('info', {}).get('funding_rate', 0.0)) * 100.0
        ...
        df_g = df_movers.sort_values(by='Değişim (%)', ascending=False).head(5).copy()
        df_l = df_movers.sort_values(by='Değişim (%)', ascending=True).head(5).copy()
        for df in [df_g, df_l]:
        ...
        return funding_rates[:5], df_g, df_l
    except:
...
def estimate_liquidation_pools(symbol):
    try:
        ...
        for i, row in df_3d.iterrows():
            for m, d in zip([0.99, 0.98, 0.96], [long_liq, long_liq, long_liq]):
                p = round((row["Dusuk"] * m) / round_step) * round_step
                d[p] = d.get(p, 0.0) + row["Hacim"]
            for m, d in zip([1.01, 1.02, 1.04], [short_liq, short_liq, short_liq]):
                p = round((row["Yuksek"] * m) / round_step) * round_step
                d[p] = d.get(p, 0.0) + row["Hacim"]
        s_long = sorted(long_liq.items(), key=lambda x: x[1], reverse=True)[:3]
        s_short = sorted(short_liq.items(), key=lambda x: x[1], reverse=True)[:3]
        s_long.sort(key=lambda x: x[0], reverse=True)
        s_short.sort(key=lambda x: x[0], reverse=False)
        return (
            pd.DataFrame([{"Likidasyon Fiyatı": f"${p:,.2f}", "Yoğunluk Derecesi": "🔴🔴🔴 YÜKSEK" if v > df_3d["Hacim"].mean()*1.5 else "🔴🔴 ORTA"} for p, v in s_long]),
            pd.DataFrame([{"Likidasyon Fiyatı": f"${p:,.2f}", "Yoğunluk Derecesi": "🟢🟢🟢 YÜKSEK" if v > df_3d["Hacim"].mean()*1.5 else "🟢🟢 ORTA"} for p, v in s_short])
        )
    except:
        return pd.DataFrame(), pd.DataFrame()

# ================= NAVİGASYON VE CÜZDAN =================
...
if app_mode == "📊 Geriye Dönük Test (Backtest)":
    ...
    bt_std = col_bt3.number_input("NW Std Sapma", min_value=1.5, max_value=4.0, value=3.0, step=0.1, key="backtest_std_input")
    bt_tp = st.slider("Hedef Kar-Al Oranı (%)", min_value=0.2, max_value=5.0, value=1.0, step=0.1, key="bt_tp_slider") / 100.0
    bt_sl = st.slider("Stop-Loss Oranı (%)", min_value=0.5, max_value=10.0, value=2.0, step=0.1, key="bt_sl_slider") / 100.0
    ...
    if st.button("▶️ Geriye Dönük Testi Çalıştır", key="backtest_run_button"):
        with st.spinner("Veriler analiz ediliyor..."):
            try:
                ...
                for i, row in df_bt.iterrows():
                    ...
                    if sum(l_status) > 0:
                        ...
                        if l_status[2] and close <= (df_bt.at[i, "NW_Alt_K3"] * (1 - bt_sl)):
                            ...
                            trade_logs.append({"Tür": "LONG STOP-LOSS", "Kapanış Zamanı": t_time, "Giriş Fiyatı": l_avg_price, "Kapanış Fiyatı": close, "Kar/Zarar ($)": (l_crypto * close) - l_usd_spent, "Kalan Bakiye": balance})
                            l_crypto, l_usd_spent, l_avg_price, l_status = 0.0, 0.0, 0.0, [False, False, False]
                        elif close >= l_tp_target:
                            balance += l_crypto * close
                            trade_logs.append({"Tür": "LONG KAR-AL", "Kapanış Zamanı": t_time, "Giriş Fiyatı": l_avg_price, "Kapanış Fiyatı": close, "Kar/Zarar ($)": (l_crypto * close) - l_usd_spent, "Kalan Bakiye": balance})
                            l_crypto, l_usd_spent, l_avg_price, l_status = 0.0, 0.0, 0.0, [False, False, False]
                ...
                df_equity, df_trades = pd.DataFrame({"Zaman": df_bt["Zaman"], "Bakiye": equity_curve}), pd.DataFrame(trade_logs)
                if not df_trades.empty:
                    st.metric("Kazanma Oranı", f"%{(len(df_trades[df_trades['Kar/Zarar ($)'] > 0]) / len(df_trades)) * 100.0:.1f}")
                    fig_bt = go.Figure()
                    ...
                    fig_bt.update_layout(title="Bakiye Gelişimi", template="plotly_dark", height=400)
                    st.plotly_chart(fig_bt, use_container_width=True, key="backtest_chart_p")
                    st.dataframe(df_trades)
                else:
                    st.warning("Hiç işlem gerçekleşmedi.")
    ...
    try:
        ...
        # 1m veri üzerinden resample hesaplamaları
        ...
        # Diğer zaman dilimleri resample edilir
        dfs = {}
